...
function supabaseAdmin() {
  ...
  if (!serviceRoleKey) {
    throw new Error(
      "Missing SUPABASE_SERVICE_ROLE_KEY environment variable. This key is required for admin operations. Find it in your Supabase project settings under API > service_role key."
    );
  }
...
__name(supabaseAdmin, "supabaseAdmin");

// src/lib/jobs/trigger.ts
init_esm();

// src/lib/jobs/client.ts
init_esm();
function toTriggerTaskId(type) {
...
__name(toTriggerTaskId, "toTriggerTaskId");
async function dispatchJob(params) {
...
__name(dispatchJob, "dispatchJob");

// src/lib/jobs/trigger.ts
function getJobsTable() {
  return supabaseAdmin().from("jobs");
}
__name(getJobsTable, "getJobsTable");
function toJobResult(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return null;
  }
  return value;
}
__name(toJobResult, "toJobResult");
async function triggerJob(params) {
  const { data: job, error } = await getJobsTable().insert({
  ...
  try {
    await dispatchJob({
      type: params.type,
      jobId: job.id,
      payload: params.input ?? void 0
    });
  } catch (error2) {
    const message = error2 instanceof Error ? error2.message : "Failed to dispatch job";
    await failJob(job.id, message);
    throw error2;
  }
  return { jobId: job.id };
}
__name(triggerJob, "triggerJob");
async function updateJobProgress(jobId, progress) {
  const { error } = await getJobsTable().update({
    status: "processing",
    progress,
...
//# sourceMappingURL=chunk-SWWEV3XL.mjs.map
