...
function buildCallbackRedirect(next = "/app") {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const response = await fetch("/api/auth/oauth/google/start", {
        ...
        body: JSON.stringify({
          redirectTo: buildCallbackRedirect(postAuthRedirectTo),
        }),
      });
      ...
      if (!response.ok || !data?.url) {
        throw new Error(
          data?.error ||
            "Google sign-in isn't configured yet. Please check your Supabase Auth provider settings."
        );
      }
